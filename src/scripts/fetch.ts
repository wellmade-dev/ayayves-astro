...
export async function fetchProducts() {
  ...
  const snipcartResponse = await fetch(`https://app.snipcart.com/api/products/`, {
    headers: {
  ...
  const products = strapiData.map((product) => createProductObject(product, snipcartData));
  return products;
}

export function createProductObject(strapiProductData: ProductAPI, snipcartData: ProductsSnipcartAPI) {

  let variantTypes: VariantGroupData[] | undefined = strapiProductData.attributes.variants;
  ...
  if (variantTypes && variantTypes.length > 0) {
    ...
    variantTypes = variantTypes.map((variantType) => {
      ...
      const variantString = variantType.variant.map((variant) => {
  ...
  const snipcartProductIndex = snipcartData.items.findIndex(item => item.userDefinedId === String(strapiProductData.id));
    ...
    if (snipcartProduct.variants && snipcartProduct.variants.length > 0) {
      snipcartProduct.variants.forEach((variant) => {
        if (variant.stock > 0) {
          ...
          const productVariantGroup = variantTypes?.find( // Find the Product Variant Group that matches the Snipcart Variant Group
            variantGroup => variantGroup.variant_type === snipcartVariantGroup
          );

          if (!productVariantGroup) return;

          const productVariantOption = productVariantGroup.variant.find(
            (variant) => variant.variant_name === snipcartVariantOption
          );
